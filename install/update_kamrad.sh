...
# КАМРАД Update Script

###################################################################################################################################################################################################

# Script                | КАМРАД Update Script
# Version               | 1.0.1
# Author                | Yumash
...
get_update_confirmation(){
  read -p "This script will update КАМРАД and its dependencies on your machine. No data loss is expected, but you should always back up your data before proceeding. Are you sure you want to continue? (y/n): " choice
  case "$choice" in
    y|Y )
...
ensure_docker_installed_and_running() {
  if ! command -v docker &> /dev/null; then
    echo -e "${RED}#${RESET} Docker is not installed. This is unexpected, as КАМРАД requires Docker to run. Did you mean to use the install script instead of the update script?"
    exit 1
  fi
...
ensure_docker_compose_file_exists() {
  if [ ! -f "/opt/kamrad/compose.yml" ]; then
    echo -e "${RED}#${RESET} compose.yml file not found. Please ensure it exists at /opt/kamrad/compose.yml."
    exit 1
  fi
}
...
force_recreate() {
  ...
  if ! docker compose -p kamrad -f /opt/kamrad/compose.yml pull; then
  ...
  if ! docker compose -p kamrad -f /opt/kamrad/compose.yml up -d --force-recreate; then
