...
# КАМРАД - Disk Info Collector Sidecar
#
# Reads host block device and filesystem info via the /:/host:ro,rslave bind-mount.
# No special capabilities required. Writes JSON to /storage/kamrad-disk-info.json, which is read by the admin container.
...
if [[ ! -f /storage/kamrad-disk-info.json ]]; then
    echo '{"diskLayout":{"blockdevices":[]},"fsSize":[]}' > /storage/kamrad-disk-info.json
...
while true; do
    ...
    cat > /storage/kamrad-disk-info.json.tmp << EOF
{
...
EOF
    ...
    if mv /storage/kamrad-disk-info.json.tmp /storage/kamrad-disk-info.json; then
        log "Disk info updated successfully."
    else
        log "ERROR: Failed to move temp file to /storage/kamrad-disk-info.json"
    fi
