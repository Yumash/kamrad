...
# КАМРАД — Disk Collector Migration Script
#
# Script                | КАМРАД Disk Collector Migration Script
# Version               | 1.0.0
# Author                | Yumash
# Website               | https://github.com/Yumash/kamrad
#
# PURPOSE:
...
#   /opt/kamrad/storage/kamrad-disk-info.json, which the admin container
...
NOMAD_DIR="/opt/kamrad"
COMPOSE_FILE="${NOMAD_DIR}/compose.yml"
COMPOSE_PROJECT_NAME="kamrad"
...
check_confirmation() {
  echo -e "${YELLOW}#${RESET} This script migrates your КАМРАД installation from the"
  ...
  echo -e "${RED}#${RESET} STOP: If you have customized your compose.yml or КАМРАД's storage setup (not common), please make these changes manually instead of using this script!\n"
...
check_compose_file() {
  if [[ ! -f "$COMPOSE_FILE" ]]; then
    ...
    echo -e "${RED}#${RESET} КАМРАД does not appear to be installed or compose.yml is missing."
    exit 1
  fi
...
stop_old_host_process() {
  local pid_file="${NOMAD_DIR}/kamrad-collect-disk-info.pid"
...
remove_old_bind_mount() {
  if ! grep -q 'kamrad-disk-info\.json' "$COMPOSE_FILE"; then
    echo -e "${GREEN}#${RESET} Old /tmp/kamrad-disk-info.json bind-mount not found — already removed.\n"
    return 0
  fi

  echo -e "${YELLOW}#${RESET} Removing old /tmp/kamrad-disk-info.json bind-mount from admin volumes..."
  sed -i '/\/tmp\/kamrad-disk-info\.json:\/app\/storage\/kamrad-disk-info\.json/d' "$COMPOSE_FILE"

  if grep -q 'kamrad-disk-info\.json' "$COMPOSE_FILE"; then
    echo -e "${RED}#${RESET} Failed to remove old bind-mount from compose.yml. Please remove it manually:"
    echo -e "${WHITE_R}      - /tmp/kamrad-disk-info.json:/app/storage/kamrad-disk-info.json${RESET}"
    exit 1
  fi
...
add_disk_collector_service() {
  ...
  awk '/^volumes:/{
    ...
    print "    image: ghcr.io/yumash/kamrad-disk-collector:latest"
    print "    pull_policy: always"
    print "    container_name: kamrad_disk_collector"
    print "    restart: unless-stopped"
    print "    volumes:"
    print "      - /:/host:ro,rslave  # Read-only view of host FS with rslave propagation so /sys and /proc submounts are visible"
    print "      - /opt/kamrad/storage:/storage  # Shared storage dir — disk info written here is read by the admin container"
    print ""
  }
...
verify_disk_collector_running() {
  sleep 3
  if docker ps --filter "name=^kamrad_disk_collector$" --filter "status=running" --format '{{.Names}}' | grep -qx "kamrad_disk_collector"; then
    ...
    echo -e "${RED}#${RESET} Check its logs with: docker logs kamrad_disk_collector"
    exit 1
  fi
}
...
# Main
echo -e "${GREEN}#########################################################################${RESET}"
echo -e "${GREEN}#${RESET}      КАМРАД — Disk Collector Migration Script                          ${GREEN}#${RESET}"
echo -e "${GREEN}#########################################################################${RESET}\n"
